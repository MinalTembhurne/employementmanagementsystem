...
const AddUser = () => {
    ...
    const handleSubmit = async (e) => {  //as we are sending the request.
        ...
        try {
            const response = await axios.post("https://dummy.restapiexample.com/api/v1/create", user);  //we are requesting POST request as we want to add the data that's why we are writing this line. & What data we are sending that's y written "user". 
            setShow(true);
        }
        ...
        <div className='container w-75 mx-auto shadow p-5'>
            ...
            { show ? <div class="alert alert-success" role="alert"> Your data has been successfully added....</div> : null}
                <div className="form-control">
                    ...
                    <input type="text" className="form-control" name="id" placeholder='Enter your unique ID...' value={id} onChange={handleChange} />
                </div>
                <div className="form-control">
                    <label className="form-label">Employee Name</label>
                    <input type="text" className="form-control" name="name" placeholder='Enter your name...' value={name} onChange={handleChange} />
                </div>
                <div className="form-control">
                    <label className="form-label">Employee Salary</label>
                    <input type="text" className="form-control" name="salary" placeholder='Enter your salary...' value={salary} onChange={handleChange} />
                </div>
                <div className="form-control">
                    <label className="form-label">Employee Age</label>
                    <input type="text" className="form-control" name="age" placeholder='Enter your age...' value={age} onChange={handleChange} />
                </div>
                <div>
